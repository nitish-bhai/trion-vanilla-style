...
const SmartSearch: React.FC<SmartSearchProps> = ({
  ...
  return <motion.div className="bg-card rounded-lg shadow-lg p-6 mb-8" initial={{
    y: -20,
    opacity: 0
  }} animate={{
    y: 0,
    ...
    duration: 0.6
      ...
      <div className="flex justify-center mb-6">
        <div className="bg-muted rounded-full p-1 flex">
          <button onClick={() => setIsPromptMode(true)} className={`px-6 py-2 rounded-full transition-all ${isPromptMode ? 'bg-primary text-primary-foreground shadow-md' : 'text-muted-foreground'}`}>
            Smart Search
          </button>
      ...
      <AnimatePresence mode="wait">
        {isPromptMode ? <motion.div key="prompt" initial={{
        opacity: 0,
        x: -20
      }} animate={{
        opacity: 1,
        ...
        duration: 0.3
            ...
            <div className="relative">
              <div className="relative">
                <Search className={`absolute left-4 top-1/2 transform -translate-y-1/2 text-muted-foreground transition-all ${isAnimating ? 'animate-spin' : ''}`} size={20} />
                <input type="text" value={prompt} onChange={e => setPrompt(e.target.value)} onKeyPress={e => e.key === 'Enter' && handlePromptSearch(prompt)} placeholder="PROMPT What You Need - e.g., 'Red cotton t-shirt in medium size'" className="w-full pl-12 pr-16 py-4 text-lg border-2 border-primary/20 rounded-full bg-background focus:border-primary focus:ring-4 focus:ring-primary/10 transition-all" />
                <button onClick={() => handlePromptSearch(prompt)} disabled={!prompt.trim() || isAnimating} className="absolute right-2 top-1/2 transform -translate-y-1/2 trion-btn px-6 py-2 disabled:opacity-50 disabled:cursor-not-allowed">
                  {isAnimating ? <div className="trion-spinner w-4 h-4"></div> : 'Search'}
                </button>
              </div>
              
              {/* Smart suggestions */}
              <div className="mt-4 flex flex-wrap gap-2">
                {['Black formal shirt', 'Red cotton t-shirt medium', 'Blue jeans under ₹2000', 'Summer dresses', 'Gym shorts'].map(suggestion => <button key={suggestion} onClick={() => {
              setPrompt(suggestion);
              handlePromptSearch(suggestion);
            }} className="px-3 py-1 text-sm bg-muted hover:bg-primary hover:text-primary-foreground rounded-full transition-all">
                    {suggestion}
                  </button>)}
              </div>
            </div>
        ...
        duration: 0.3
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-4">
              ...
              <div>
                ...
                <select value={filters.category} onChange={e => setFilters(prev => ({
              ...prev,
              ...
              <div>
                ...
                <div className="flex gap-2">
                  <input type="range" min="0" max="5000" value={filters.priceRange[1]} onChange={e => setFilters(prev => ({
                ...prev,
              ...
              <div>
                ...
                <select value={filters.size} onChange={e => setFilters(prev => ({
              ...prev,
              size: e.target.value
            }))} className="w-full p-2 border border-border rounded-md bg-background">
              ...
              <div>
                ...
                <select value={filters.color} onChange={e => setFilters(prev => ({
              ...prev,
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4">
              ...
              <div>
                ...
                <select value={filters.material} onChange={e => setFilters(prev => ({
              ...prev,
              ...
              <div>
                ...
                <select value={filters.gender} onChange={e => setFilters(prev => ({
              ...prev,
              ...
              <div>
                ...
                <select value={filters.minRating} onChange={e => setFilters(prev => ({
              ...prev,
              ...
              <div className="flex items-center">
                <label className="flex items-center space-x-2">
                  <input type="checkbox" checked={filters.hasDiscount} onChange={e => setFilters(prev => ({
                ...prev,
      ...
      <div className="flex justify-center mt-6">
        <button onClick={clearFilters} className="flex items-center gap-2 px-4 py-2 text-sm text-muted-foreground hover:text-foreground border border-border rounded-full hover:bg-muted transition-all">
          <X size={16} />
          Clear All Filters
        </button>
      </div>
